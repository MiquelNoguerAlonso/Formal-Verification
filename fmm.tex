...
For equal-length allocation lists, write $A\oplus B:=\lean{List.zipWith (fun a b => a+b) A B}$ for componentwise addition. Every displayed sum of allocation vectors below uses $\oplus$.
...
$(f\rhd g)(x)=(A_f\mathbin{+\!\!+}A_g,\,x'_g)$ where $(A_f,x'_f)=f(x)$ and $(A_g,x'_g)=g(x'_f)$. The identity allocates nothing. A ladder is the right fold of its level operators under $\rhd$.
\end{definition}
...
=\lean{priceTime}\,Q\,x
\oplus\lean{priceTime}\,(Q-\lean{priceTime}\,Q\,x)\,y.
\]
...
Exchange documentation specifies that parity allocation proceeds from an allocation wheel and that the wheel advances across executions \citep{nyse2024parity}. Theorems~\ref{thm:pointer-band} and~\ref{thm:pointer-full} place balance and exact split/combined equality on one executable construction. Example~\ref{thm:pointer-necessary}, Theorem~\ref{thm:fiber-pointer}, and Theorem~\ref{thm:saturated-quotient} below use explicit, different observation contracts: the four-party collision requires pointer information beyond queue and fills; the fiber theorem gives exact pointer necessity and sufficiency conditional on a common authenticated observation; and the $mL$ result bounds a complete predictive summary without separate side inputs. Corollary~\ref{cor:allowance-recovery} proves that cumulative allocation determines the allowance on every reachable live-head state. These statements concern the modeled wheel and do not characterize all banded stream-consistent mechanisms.
...
If $m^2e=(m-1)V$ then the first-order condition holds at $O=(m-1)e$, total effort satisfies $m\cdot me=(m-1)V$, and $(m-1)(m+1)<m^2$: a fraction $(m-1)/m$ of the prize is dissipated and is strictly increasing in $m$.
\end{theorem}
...
On the grid $\{0,\dots,N\}$ with prize $N$ and a uniform opponent, the $(N+1)$-scaled doubled payoff of bid $b$ is $N-2b$; the uniform strategy's own scaled payoff is $0$; the best pure deviation improves expected payoff by less than half a cost unit, and relative to the prize the gain is less than $1/N$.
\end{theorem}
...
Rule~605 & \lean{effSpread2} & Signed $\Z$ kernel is bounded above by quoted spread, vanishes at an exact midpoint, and preserves below-midpoint magnitude. & Not by itself a side-aware reporting system; September and November profiles differ \citep{sec2024rule605,sec2026rule605faq,sec2026rule605order}. & \lean{Rule605Report}; dated profile; exhaustive field types \\[2pt]
...
\begin{itemize}[leftmargin=*]
  ...
  \item Nasdaq Rules~4702(a), 4702(b)(1)--(16), 4703(a)--(l), 4752(a)--(f), 4753(a)--(f), 4754(a)--(c), and 4757(a)--(c).
\end{itemize}
...
\lean{NasdaqOrderType4702} has all sixteen Rule~4702 order types and \lean{NasdaqAttribute4703} all twelve Rule~4703 attribute families. Their exhaustiveness theorems establish list completeness. \lean{VenueBookState} covers session eligibility, type semantics, attribute compatibility, Reg NMS and Reg SHO repricing, timestamp behavior, priority, routing, data and fallback sources, locks, self-help, cancellation, and re-entry. \lean{NasdaqCrossState} covers definitions, eligible interest, imbalance data and schedules, price hierarchy, collars, allocation priority, uniform price, official-price dissemination, LULD/hybrid variants, and contingencies.
